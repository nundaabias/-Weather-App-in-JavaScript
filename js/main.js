...
const messageAdd = (message, classAdd) => {
  txtInfo.innerHTML = message;
  infoBox.classList.toggle(classAdd);
};
...
btnSubmit.addEventListener("click", () => {
  if (txtCity.value != "") {
    ...
    timeout = setTimeout(messageAdd('Please add a city name!', 'error'), 5000);
  }
});
...
const waetherDetails = (info) => {
  ...
  if (info.cod == "404") {
    ...
    wrapper.querySelector(".app-footer .feels").innerText =
      Math.floor(feels_like);
    wrapper.querySelector(".app-footer .humidity").innerText =
      Math.floor(humidity);
